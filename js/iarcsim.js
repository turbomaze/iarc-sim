...
var IARCSim = (function () {
  ...
  function initGameState () {
    ...
    // obstacle init
    obstacles = []
    for (ai = 0; ai < OBST_SPEC.N; ai++) {
      theta = ai * (2 * Math.PI / OBST_SPEC.N)
      x = OBST_SPEC.initD * Math.cos(theta)
      y = OBST_SPEC.initD * Math.sin(theta)
      obstacles.push(
        new ObstacleRoomba(
          [CENTER[0] + x, CENTER[1] + y],
          'red'
  ...
  function controlUav () {
    var closestRoomba = roombas.filter(function (r) {
      var angle = Math.atan2(r.direc[1], r.direc[0])
      var desired = 0
      return Math.abs(angle - desired) > Math.PI / 4
    }).reduce(function (a, r, i) {
      if (a === null) {
        return [r, i]
      } else {
        var aDist = a[0].distTo(uav)
        var rDist = r.distTo(uav)
        return aDist > rDist ? [r, i] : a
      }
    }, null)
    goTowardsRoomba(closestRoomba[1])
    var angle = Math.atan2(closestRoomba[0].direc[1], closestRoomba[0].direc[0])
    var desired = 0
    var tap = Math.abs(angle - desired) > Math.PI / 4
    if (tap) tapRoomba()
  }
  ...
  function drawBoard () {
    ...
    for (ai = 0; ai < NUM_GRID_LINES[1]; ai++) {
